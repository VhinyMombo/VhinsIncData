...
# Load the shapefile
gabon_parks_sf <- st_read(shapefile_path, quiet = TRUE)


gabon_parks_sf <- gabon_parks_sf[, c("NAME", "DESIG_TYPE", "DESIG_ENG")]
# plot(gabon_parks_sf)
...
map <- map %>% 
  ...
  addLayersControl(
    baseGroups = c("Esri World Imagery", "Open Street Map"), # Tile providers
...
Gabon's national parks represent the country's ongoing efforts to preserve its rich biodiversity and stunning natural beauty. These protected areas provide sanctuary for countless species and are critical for maintaining the ecological balance of the region. By safeguarding these unique ecosystems, Gabon ensures that future generations can enjoy and benefit from the country's natural heritage.

**Explore Gabon's National Parks!** Visit these incredible parks to witness nature at its most pristine, where wildlife thrives and ecosystems remain largely untouched by human development.
...
gabon_GDP$second_derivative_color <- ifelse(gabon_pop$second_variation < 0, "red", "green")
